...
Version 0.1.0-demo.7 allows live reading/writing corrections from the examination clock, always advertises the classroom-network student address (never `127.0.0.1`), and raises a dashboard notification when a candidate leaves the exam window, on top of demo.6's focus-loss audit telemetry and live alerts, root redirect, right-click suppression, and single English B Paper 2 Reading familiarisation example. See the [release notes](release/RELEASE_NOTES-0.1.0-demo.7.md) for verification limits and update instructions.
