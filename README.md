...
![PacePaper live candidate work](docs/user-guide/screenshots/live-work.png)

While a sitting is live, **Watch work** follows each candidate's saved answers, drawings, and notepad as they write.
...
- Live supervision while a sitting runs: **Watch work** on a live session shows each candidate's saved answers, drawings, and notepad, refreshing as they type — read-only, and visible only to the teacher
...
**Version 0.1.0-demo.13 is prepared for Windows x64 and Linux x64** — a teacher can watch candidate work live during a sitting: **Watch work** on any live session shows each candidate's saved answers, drawings, and notepad, refreshing as they type, and read-only. Candidates are told at sign-in and in the waiting room that their teacher can see their work. It also keeps demo.12's candidate rehearsal: a teacher can rehearse any paper as a student: **Preview as student** in the Paper library opens the real candidate interface with the paper's own timings, phases, locked sections, and audio, and records nothing. It keeps demo.11's classroom address chosen from a real network adapter rather than a container or virtual adapter, so a machine running Docker, Hyper-V, or WSL advertises the interface that student devices can actually reach. It keeps demo.10's classroom sharing active from launch, demo.9's teacher **Settings** area, independent **light/dark themes**, the **IB MYP eAssessment** exam system, **video and YouTube stimulus material**, and the candidate **full-screen** suggestion, plus the demo-era strengths: live reading/writing timing corrections from the examination clock, focus alerts when a candidate leaves the exam window, and one generic **Sample paper** so a fresh install is usable immediately. A macOS app remains withheld for signing/notarization reasons; see [release notes](release/RELEASE_NOTES-0.1.0-demo.11.md).
...
Each published build is summarised in its release notes; see the [current release notes](release/RELEASE_NOTES-0.1.0-demo.13.md) for what ships and the verification limits.
