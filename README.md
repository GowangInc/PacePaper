...
**Version 0.1.0-demo.8 is prepared for Windows x64 and Linux x64**, the first under the PacePaper name. It ships with the new linework beat-arc icon and seeds one generic **Sample paper** with no subject, level, or IB branding, so new users can try the whole exam workflow before building their own. It keeps the demo.7 capabilities: the teacher can correct reading and writing minutes from the examination clock even while an exam is live (candidate phase boundaries and deadlines move immediately), the dashboard and clock always advertise the classroom-network address students should join rather than a loopback `127.0.0.1`, and the dashboard raises a visible notification when a candidate leaves the exam window. A new macOS app is withheld because Apple signing/notarization credentials are not configured. Mac users can run the updated source using `Start PacePaper.command` or `bun run start:app`; do not use the earlier unsupported Mac downloads. Use the [GitHub release page](https://github.com/GowangInc/PacePaper/releases/tag/v0.1.0-demo.8) for the draft assets and verification status.
...
Version 0.1.0-demo.8 introduces the PacePaper name with a linework beat-arc icon and a generic preloaded **Sample paper** (no subject or IB branding) that demonstrates the exam workflow, on top of demo.7's live reading/writing clock corrections, classroom-network student address, and focus-loss notifications, demo.6's focus-loss audit telemetry and live alerts, root redirect, and right-click suppression. See the [release notes](release/RELEASE_NOTES-0.1.0-demo.8.md) for verification limits and update instructions.
