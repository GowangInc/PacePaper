...
- A 52-paper original development library: 34 IB-oriented examples, 15 full-length non-IB mocks, and three short AP walkthroughs. Standalone demo.6 packages only its single English B familiarisation example.
...
The current release builder keeps data outside the replaceable executable, in the per-user application-data location for the platform. It does not package a live database, teacher paper, student response, or protected reference material. A fresh demo.6 release seeds one original IB-aligned English B Paper 2 Reading familiarisation example; teacher-created and imported papers remain in the local library. Previously downloaded apps retain the code and examples from their own release; source edits do not update those downloads. The teacher login remains `admin` / `admin` and is reset on every app launch; it is not suitable for real student data.
...
Version 0.1.0-demo.6 adds browser focus-loss audit telemetry and expandable live teacher alerts, redirects the app root to student sign-in, suppresses browser right-click as a classroom deterrent, and packages one English B Paper 2 Reading familiarisation example. See the [release notes](release/RELEASE_NOTES-0.1.0-demo.6.md) for verification limits and update instructions.
