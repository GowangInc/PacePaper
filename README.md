...
**Version 0.1.0-demo.10 is prepared for Windows x64 and Linux x64** — classroom sharing is now active as soon as PacePaper starts, so student devices on the same network can join without a dashboard click. The app reuses the classroom address it saved last time and, on a fresh install, chooses and saves the machine's first private network address; the student address shown on the dashboard is always an address the server accepts. It keeps demo.9's teacher **Settings** area (change the saved teacher password), the independent **light/dark themes**, the **IB MYP eAssessment** exam system, **video and YouTube stimulus material**, and the candidate **full-screen** suggestion, plus the demo-era strengths: live reading/writing timing corrections from the examination clock, focus alerts when a candidate leaves the exam window, and one generic **Sample paper** so a fresh install is usable immediately. A macOS app remains withheld for signing/notarization reasons; see [release notes](release/RELEASE_NOTES-0.1.0-demo.10.md).
...
Each published build is summarised in its release notes; see the [current release notes](release/RELEASE_NOTES-0.1.0-demo.10.md) for what ships and the verification limits.
