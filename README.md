...
**Version 0.1.0-demo.9 is prepared for Windows x64 and Linux x64** — the first downloadable build to include the teacher **Settings** area (change the saved teacher password) and the independent **light/dark themes** for the teacher and candidate screens. It adds an **IB MYP eAssessment** exam system to the Paper Builder, **video and YouTube stimulus material** for papers (embedded player, or direct https video URLs), and a full-screen suggestion on the candidate exam screen. It keeps the demo-era strengths: live reading/writing timing corrections from the examination clock, the classroom-network student address, focus alerts when a candidate leaves the exam window, and one generic **Sample paper** so a fresh install is usable immediately. A macOS app remains withheld for signing/notarization reasons; see [release notes](release/RELEASE_NOTES-0.1.0-demo.9.md).
