...
Version **0.1.0-demo.11** targets Windows x64 and Linux x64, under the PacePaper name. There is no Mac application in this release: Apple signing and notarization credentials are not configured. Mac users can run the source as described below. A previously downloaded app retains its own code and examples; check the release notes supplied with that download.
...
- macOS — run the source for now; no Mac archive is included in demo.11
...
The current source is labelled `0.1.0-demo.11`, and a fresh release installs the generic **Sample paper** described above. See `RELEASE_NOTES-0.1.0-demo.11.md` for the dated review record. Later source edits require a new build and verification; existing downloads are unchanged.
